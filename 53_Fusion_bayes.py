...
# Constants for Optuna optimization
N_TRIALS = 1000  # Number of trials for Optuna
TIMEOUT = 33000  # Timeout for Optuna optimization in seconds
...
def objective(trial):
    ...
    batch_size = trial.suggest_int('batch_size', 8, 64)  # Batch size optimization (8-64)
...
def save_top_n_results(study, n, filepath):
    ...
    for trial in top_n_trials:
        params = {
            ...
            "batch_size": trial.params.get("batch_size")  # Include batch size in results
        }
...
def main():
    ...
    # Record the end time of optimization
    optimization_end_time = time.time()

    # Calculate the total optimization time
    total_optimization_time = optimization_end_time - optimization_start_time
    console.print(f"[bold green]Optimization Completed in {total_optimization_time:.2f} seconds.[/bold green]")
