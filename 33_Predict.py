...
with open(results_file, 'a') as result_file:
    for config_idx, config in enumerate(lstm_configs):
        ...
        history = model.fit(X_train, y_train, epochs=8500, batch_size=48, callbacks=[early_stopping], verbose=0)
        end_time = time.time()
...
top_min_config_file = find_trimmed_filename(top_min_config_base, input_folder)

try:
    huber_configs = load_top_min_configs(os.path.join(input_folder, top_min_config_file))
